...
initRFC = RandomForestClassifier(max_depth=3, n_estimators=10)
initRFC.fit(xtrain,ytrain)
print('Initial Random Forest Classifier Accuracy: ' ,initRFC.score(xtest,ytest))

Ss=[]
for i  in range(10,50,5):
    model=RandomForestClassifier(n_estimators=i , max_depth=None)
...
plt.title('RFC accuacy vs n estimators')
plt.show()

model=gscv(RandomForestClassifier(),
...
savegscv = resRFC.to_csv('gscvRFCUNWEIGHTED.csv', index=True)

bestRTC = RandomForestClassifier(n_estimators=15,max_depth=10)
...
plt.xlabel('accuarcy')
plt.title('K-Nearestneighbor accuracy')
plt.show()
maxKNNAC = 0
maxKNNat = 0
...
max_iter=300,learning_rate_init=.05),
...
# print(resMLP)
savegscv = resMLP.to_csv('gsvMLPUNWEIGHTED.csv', index=True)

bestMLP = MLPClassifier(hidden_layer_sizes=5,activation='logistic',batch_size=30,solver='lbfgs',
random_state=0,max_iter=300,learning_rate_init=.05)
